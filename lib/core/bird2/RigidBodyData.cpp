...
RigidBodyData::RigidBodyData(const Eigen::MatrixX3d& verts, const Eigen::MatrixX4i& tets)
    : volume_(0)
{
    V = verts;
    T = tets;
...
void RigidBodyData::computeFaces()
{
    struct triple {
...
double RigidBodyData::computeVolume()
{
    double volume = 0;
...
Vector3d RigidBodyData::computeCenterOfMass()
{
    Vector3d cm(0, 0, 0);
    for (int i = 0; i < F.rows(); i++) {
        Vector3d pts[3];
...
Eigen::Matrix3d RigidBodyData::computeInertiaTensor()
{
